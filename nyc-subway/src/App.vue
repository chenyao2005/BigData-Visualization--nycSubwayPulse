...
<div v-show="tooltip.visible" class="tooltip" :style="{ left: tooltip.x + 'px', top: tooltip.y + 'px' }">
    ...
    <div v-if="showDoc" class="doc-modal-overlay" @click="showDoc = false">
      <div class="doc-modal-content" @click.stop>
        ...
        <ul>
          <li><strong>时空分布特征：</strong>早晚高峰期间，曼哈顿商业区与外围住宅区（如布鲁克林、皇后区）的客流潮汐是如何涌动的？比如早上（7-9 AM），客流极值点密集爆发于布鲁克林、皇后区等外围住宅区；而傍晚（5-7 PM），高密度客流点瞬间向曼哈顿中下城聚集。这直观地揭示了纽约市“职住高度分离”的城市空间结构特征。</li>
          <li><strong>技术普及率差异：</strong>现代移动支付技术（OMNY）与传统磁条卡（MetroCard）的使用比例在不同行政区、不同时间段是否存在显著差异？是否能反映出不同社区对新技术的接受度或通勤人群的结构差异？可以看到刷手机方式已成为主流，但是仍有少部分地区（比如曼哈顿下城区），由于有学校和著名景点，刷卡量会比其他地方多一些。这说明非接触式的手机支付已被纽约市民大范围接受，尤其是白领、商务人士等，但是一些特殊群体（如学生、游客、低收入群体等）仍会选择刷卡方式</li>
        </ul>
        ...
        <h4>替代方案与最终决策：</h4>
        <li>一开始想把进站与出站数据进行一个对比，但写完代码发现 MTA 采取单向计费，是没有出站数据的。所以调整了方向，挖掘 <code>payment_method</code> 这一更有叙事价值的维度，使得作品的社会学意义更加丰富。</li>
        <li>刚开始在如何展现数据的时候，我也考虑了要不要弄一个大的折线图，展现三种方式（总客流量、手机支付、刷卡支付）的对比，实时根据点的变化来动，但是一个是由于地图区域比较大，还有旁边的仪表盘，留给折线图的区域不多了，如果同时三方折线在小图上会看不清；一个是由于点的大小已经能够明显体现刷手机和刷卡支付的人数多少了，并且刷卡的人很少（相比于刷手机的，这样竖坐标为了平衡，刷卡人数会变成一条直线，无法体现刷卡的人流一天的趋势），所以最后采用的是，选择哪种的就展示哪种的单独折线图。</li>
       ...
       <h3>3. 外部资源引用</h3>
        ...
        <h4>开发评述：</h4>
        <ul>
          <li><strong>数据预处理与结构设计（约 8 小时）：</strong>官方 CSV 数据集非常庞大且存在冗余。我先编写了 Python 脚本进行数据清洗，按时间、站点和支付方式进行 <code>groupby</code> 与透视，并最终将其转化为对 D3 友好的嵌套 JSON 结构。同时还需要将包含行政区的 CSV 元数据与 JSON 动态数据在前端进行主键合并 (Join)。</li>
          <li><strong>D3 交互与生命周期控制（约 12 小时）：</strong>确保拖动时间滑块时，散点的 Enter/Update/Exit 动画能够流畅过渡而不卡顿。初期由于过渡动画堆叠导致浏览器渲染延迟，如果滑很快的话就会反应不过来，所以后续我优化了数据绑定并且取消了高频触发时的动画延迟，使得最后的呈现效果可以根据时间块的滑动实时反应。</li>
          <li><strong>UI 调优（约 10 小时）：</strong>为了能够使整体的展现更完善，我在ui上花费了一定的功夫，包括确定暗色主题，地图大小、缩放比例、中心位置，站点的大小、展现形式以及多视图（包括折线图、仪表盘、弹窗等）间的排版布局。其中，尤其是地图部分的确认，怎么样既展示出完整的地图板块，又展示出地铁线路、站点，综合考虑地图大小，比例，使其不太大显得占满屏幕，又不太小使得看不清。如此确保了最终界面“简洁而优雅”的设计目标。</li>
        </ul>
      </div>
    </div>
    </div> </template>
